...
busqueda_por_producto = input('Buscar: ').strip().capitalize()

# Realiza la búsqueda por código

if busqueda_por_producto.isdigit():
    # Verifica que el código sea válida
    codigo = int(busqueda_por_producto)
...
# Realiza la búsqueda por nombre
    ...
    resultados = [detalle for detalle in inventario_licores.values() if busqueda_por_producto in detalle['licor']]
    if resultados:
